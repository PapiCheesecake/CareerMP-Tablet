    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        var mapState = {
          ...
          mapImages: null, currentImageLevel: null,
        ...
        var MC = {
          ...
          waypointLine: 'rgba(59,130,246,1)', // Darker, fully opaque blue (Tailwind blue-500)
        ...
        function buildRoadPaths() {
          ...
          var scale = Math.min(cw / (b.maxX - b.minX), ch / (b.maxY - b.minY));
          var dx = (cw - (b.maxX - b.minX) * scale) / 2;
          var dy = (ch - (b.maxY - b.minY) * scale) / 2;
          ...
          for (var i = 0; i < mapState.roadData.length; i += 5) {
            ...
            if (t >= 0 && t <= 2) {
              var nx1 = (mapState.roadData[i] - b.minX) * scale + dx;
              var ny1 = (b.maxY - mapState.roadData[i+1]) * scale + dy;
              var nx2 = (mapState.roadData[i+2] - b.minX) * scale + dx;
              var ny2 = (b.maxY - mapState.roadData[i+3]) * scale + dy;
        ...
        function w2c(wx, wy) {
          ...
          var nx = (wx - b.minX) * scale + dx;
          var ny = (b.maxY - wy) * scale + dy;
          return { x: nx * mapState.zoom + mapState.offsetX, y: ny * mapState.zoom + mapState.offsetY };
        }
        ...
        function drawMap() {
          ...
          resizeMapCanvas(); // Ensure canvas is always perfectly sized before drawing
          var cw = parseInt(mapCanvas.style.width)||300, ch = parseInt(mapCanvas.style.height)||300;
          if (cw === 0 || ch === 0) { // If still hidden, skip this frame
          ...
          // Topographical Background Images
          if (mapState.mapImages && mapState.mapImages.length > 0) {
            mapCtx.save();
            // Apply global canvas smoothing for high quality scale
            mapCtx.imageSmoothingEnabled = true;
            mapCtx.imageSmoothingQuality = 'high';
            // Signature tablet theme: dark, desaturated amber map base
            mapCtx.filter = 'grayscale(100%) brightness(35%) sepia(100%) hue-rotate(345deg) saturate(200%)';
            mapCtx.globalAlpha = 0.85;
            mapState.mapImages.forEach(function(m) {
              if (m && m.img.complete && m.img.naturalWidth > 0) {
                var p1 = w2c(m.offset[0], m.offset[1]);
                var p2 = w2c(m.offset[0] + m.size[0], m.offset[1] - m.size[1]);
                // drawImage(image, dx, dy, dWidth, dHeight)
                mapCtx.drawImage(m.img, p1.x, p1.y, p2.x - p1.x, p2.y - p1.y);
              }
            });
            mapCtx.restore();
          }
          ...
          if (mapState.roadPaths) {
            // Tablet signature theme for roads
            var colors = ['rgba(232, 148, 58, 1.0)', 'rgba(232, 148, 58, 0.75)', 'rgba(232, 148, 58, 0.5)'];
            var widths = [2.0, 1.25, 0.75];
            mapCtx.save();
            ...
            for (var t = 0; t < 3; t++) {
              ...
              // When zoomed out, lines become thinner (less bold). When zoomed in, they get thicker.
              var pixelWidth = widths[t] * Math.pow(mapState.zoom, 0.6);
              // Clamp to reasonable pixel sizes so they don't vanish completely or get too massive
              pixelWidth = Math.max(0.5, Math.min(pixelWidth, widths[t] * 2.5));
              
              mapCtx.lineWidth = pixelWidth / mapState.zoom;
            ...
            if (currentTab === 'map') {
              ...
              if (!mapState.updateInterval) {
                mapState.updateInterval = setInterval(function() {
                  if (typeof bngApi !== 'undefined') bngApi.engineLua('extensions.career_careerMPTablet.getMapData()');
                }, 5000); // 5s slow data refresh (facilities, paths)
              }
              if (!mapState.fastUpdateInterval) {
                mapState.fastUpdateInterval = setInterval(function() {
                  if (typeof bngApi !== 'undefined') {
                    bngApi.engineLua('extensions.career_careerMPTablet.getPlayerPosition()', function(pos) {
                      if (pos && mapState.data) {
                        mapState.data.playerPos = pos;
                      }
                    });
                  }
                }, 100); // 10 FPS fast player position tracking
              }
            } else {
              stopMapAnim();
              if (mapState.updateInterval) { clearInterval(mapState.updateInterval); mapState.updateInterval = null; }
              if (mapState.fastUpdateInterval) { clearInterval(mapState.fastUpdateInterval); mapState.fastUpdateInterval = null; }
            }
          });
        });
        ...
        scope.$on('careerMPTabletMapData', function (event, data) {
          ...
          // Load Topographical Map Images if level changed
          if (data.levelName && mapState.currentImageLevel !== data.levelName) {
            mapState.currentImageLevel = data.levelName;
            mapState.mapImages = [];
            
            if (data.minimap && data.minimap.length > 0) {
              data.minimap.forEach(function(m, idx) {
                var img = new Image();
                var src = m.file.startsWith('/') ? m.file : '/' + m.file;
                img.onload = function() {
                  mapState.mapImages[idx] = {
                    img: img,
                    size: m.size || [2048, 2048],
                    offset: m.offset || [-1024, 1024]
                  };
                };
                img.src = src;
              });
              ...
              var img = new Image();
              var src = data.minimapImage.startsWith('/') ? data.minimapImage : '/' + data.minimapImage;
              img.onload = function() {
                var size = data.levelSize || [2048, 2048];
                var offset = [-size[0]/2, size[1]/2];
                mapState.mapImages[0] = {
                  img: img,
                  size: size,
                  offset: offset
                };

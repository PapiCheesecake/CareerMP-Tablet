...
local settingsPath = 'settings/CareerMPTablet/settings.json'
...
local function getMapData()
  ...
  if playerVeh then
    ...
    if pos then
      ...
      table.insert(allPositions, {x = pos.x, y = pos.y})
    end
  end
  ...
  if mapData and mapData.nodes then
    for nid, n in pairs(mapData.nodes) do
      for lid, data in pairs(n.links) do
        ...
        -- Deduplicate bidirectional edges to halve payload size
        if tostring(nid) > tostring(lid) then goto continue end
        
        local n2 = mapData.nodes[lid]
        if n2 then
          local drivability = data.drivability or 1
          local roadType = 1 -- Normal
          if drivability < 0.25 then roadType = 3 -- Dirt/Trail
